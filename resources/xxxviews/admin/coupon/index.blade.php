		...
		<div class="box-header with-border">
			...
			<div class="box-tools pull-right">
				@can('create', App\Coupon::class)
					<a href="#" data-link="{{ route('admin.promotion.coupon.create') }}" class="ajax-modal-btn btn btn-success btn-flat">Add Coupons</a>
				@endcan
			</div>
		</div> <!-- /.box-header -->
		<div class="box-body">
			<table class="table table-hover table-option">
				<thead>
					<tr>
						...
						<th>{{ trans('app.restricted') }} (locations)</th>
				...
				@foreach($coupons as $coupon )
					...
					@php $location = DB::table('in_location')->where('id',$coupon->location)->get(); @endphp
					<tr>
						<td>
							{{ $coupon->name }}
						...
						<td>
							<strong>
								{{-- $coupon->type == 'amount' ? round($coupon->value) : get_formated_decimal($coupon->value) . ' ' . trans('app.percent') --}}

								@if($coupon->type == 'amount')
								    {{round($coupon->value)}} KSh
								@else
								    {{get_formated_decimal($coupon->value)}} %
								@endif
							</strong>
						</td>
						<td>
