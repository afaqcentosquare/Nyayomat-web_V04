...
@section('buttons')
  @can('create', App\Order::class)
    <a href="#" data-link="{{ route('admin.order.order.searchCutomer') }}" class="ajax-modal-btn btn btn-success btn-flat">{{ trans('app.search_again') }}</a>
  @endcan

  @can('index', App\Order::class)
    <a href="{{ route('admin.order.order.index') }}" class="btn btn-success btn-flat">{{ trans('app.cancel') }}</a>
  @endcan
@endsection
          ...
          <div class="box-body">
            ...
            <div class="box-tools pull-right">
              @if(Gate::allows('create', App\Cart::class) || Gate::allows('create', App\Order::class) || Gate::allows('update', App\Cart::class))
                <button name='action' value="1" id="saveTheCart" class='btn btn-flat btn-lg btn-success' >
              ...
              @if( $shipping_options != 'NaN' && ! isset($order_cart) && Gate::allows('create', App\Order::class))
                <button name='action' type="submit" class='btn btn-flat btn-lg btn-success' >
      ...
      <div class="col-md-3 nopadding-left">
        <div class="box">
          ...
          <div class="box-body">
            ...
            @can('view', $customer)
              <a href="#" data-link="{{ route('admin.admin.customer.show', $customer->id) }}" class="ajax-modal-btn btn btn-success btn-xs">{{ trans('app.view_detail') }}</a>
            @endcan
          </div>
        </div>
        ...
        <div class="box">
          ...
          <div class="box-body">
            ...
            @else
              ...
              @else
                <a href="#" data-link="{{ route('address.create', ['customer', $customer->id]) }}" class="ajax-modal-btn btn btn-success"><i class="fa fa-plus-square-o"></i> {{ trans('app.add_address') }} </a>
              @endif
            @endif
  ...
  <script language="javascript" type="text/javascript">
    ;(function($, window, document) {
      ...
      if(billing_address.length == 0){
        $("#global-alert-msg").html('{!! trans('messages.notice.no_billing_address') . ' ' . '<a class="ajax-modal-btn btn btn-success" href="#" data-link="' . route('address.create', ['customer', $customer->id]) . '"><i class="fa fa-plus-square-o"></i>' . trans('app.add_address') . '</a>' !!}');
        $("#global-alert-box").removeClass('hidden');
      }
      ...
      var apply_btn = '<div class="spacer5"></div><button class="popover-submit-btn btn btn-flat btn-success btn-lg btn-block" type="button">{{ trans('app.apply') }}</button>';
