    ...
    <div class="box">
      <div class="box-header with-border">
          ...
          <div class="box-tools pull-right">
            @if(!isset($product))
              <a href="#" data-link="{{ route('admin.catalog.product.upload') }}" class="ajax-modal-btn btn btn-success btn-flat">{{ trans('app.bulk_import') }}</a>
            @endif
          </div>
      </div> <!-- /.box-header -->
      <div class="box-body">
    ...
    <div class="box">
      ...
      <div class="box-body">
        ...
        <fieldset>
          ...
          <!--    <div class="help-block with-errors"></div>-->
          <!--</div>-->
          @php $lo = DB::table('in_location')->where("deleted_at",null)->get(); @endphp
          <div class="form-group">
              <label>Location*</label>
              <select class="form-control select2" multiple  name="origin_country[]" required>
                  @foreach($lo as $l)
                    <option value="{{$l->id}}"
                      @if(isset($product) && in_array($l->id, $product->locations->pluck('id')->toArray())) selected @endif>{{$l->name}}</option>
                  @endforeach
              </select>
          </div>
          ...
          <div class="form-group">
            {!! Form::label('model_number', 'Product Code', ['class' => 'with-help']) !!}
            <i class="fa fa-question-circle" data-toggle="tooltip" data-placement="top" title="Product Code"></i>
            {!! Form::text('model_number', null, ['class' => 'form-control', 'placeholder' => 'Product Code']) !!}
          </div>
