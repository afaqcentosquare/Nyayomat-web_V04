            ...
            <table id="mytableID" class="table">
                ...
                <tbody class="accordion" id="groupDescription">
                    @foreach ($engaged_assets as $key => $engaged_asset)                                                                                        
                        <tr>
                            <td nowrap>
                                {{\Illuminate\Support\Str::upper($engaged_asset->asset_name)}}
                            </td>
                            <td>
                                        ...
                                        <tbody>
                                            ...
                                            @php
                                                $merchant_info = App\Models\MerchantAssetOrder::where('merchant_id',$engaged_transaction->merchant_id)->where("asset_id", $engaged_transaction->asset_id)
                                                                                                ->join("shops", "shops.owner_id", "tbl_acp_merchant_asset_order.merchant_id")
                                                                                                ->selectRaw("tbl_acp_merchant_asset_order.id,shops.name,sum(total_out_standing_amount) as total_out_standing_amount,(sum(units) * unit_cost) - sum(total_out_standing_amount) as total_paid,(sum(units) * unit_cost) as total_cost, sum(installment) as total_installment")
                                                                                                ->where("tbl_acp_merchant_asset_order.status", "delivered")
                                                                                                ->first();
                                                    ...
                                                    <td>
                                                        <ul class="list-unstyled">
                                                            <li class="font-weight-bold">
                                                                {{\Illuminate\Support\Str::upper($merchant_info->name)}}
                                                            </li>
                                                            <li>
                                                                {{-- @if ($w %7 != 0) --}}
                                                        ...
                                                        <div class="progress rounded-lg">
                                                            @php
                                                                if($merchant_info->total_out_standing_amount != 0){
                                                                    $progress = ($merchant_info->total_paid/$merchant_info->total_cost) * 100;
                                                                }else{
                                                                    $progress = 100;
                                                                }
