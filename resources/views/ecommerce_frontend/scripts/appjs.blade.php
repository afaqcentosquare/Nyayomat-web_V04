...
(function($, window, document) {
    ...
    $(".back_btn").click(function() {
        ...
        $("#show_len").on('click', function(){

            if($(".len_location").val()==""){
                @include('ecommerce_frontend.layouts.notification', ['message' => 'please select location', 'type' => 'warning', 'icon' => 'check-circle'])
                return false;
            } else {
                return true;
            }
            ...
            $.confirm({
                ...
                buttons: {
                  'confirm': {
                      ...
                      action: function () {
                        ...
                        if (typeof url != 'undefined') {
                            ...
                            form.submit();
                            @include('ecommerce_frontend.layouts.notification', ['message' => trans('theme.notify.confirmed'), 'type' => 'success', 'icon' => 'check-circle'])
                        }
                        return true;
                      }
                  },
                  'cancel': {
                      ...
                      btnClass: 'btn-success flat',
                      action: function () {
                        @include('ecommerce_frontend.layouts.notification', ['message' => trans('theme.notify.canceled'), 'type' => 'warning', 'icon' => 'times-circle'])
                      }
                  },
                }
            });
        ...
        $("a.viewSwitcher").bind("click", function(e){
            ...
            if($(this).hasClass('btn-success')){
                ...
                $('.viewSwitcher').toggleClass('btn-primary btn-success');
                ...
                if($('.product-list .product').attr('class')=="product sc-product-item product-list-view"){
                    $('.product-list .product').css("height","300px");
                    $('.product-list .col-md-12').css("height","300px");
                }
                // Change the action buttons
                $('.product-actions').toggleClass('btn-group');
                $('.product-actions a.btn-success, .product-actions a.btn-link').toggleClass('btn-sm');
                $('.product-actions a:first-child').toggleClass('btn-link btn-success');
            }
            return false;
        });
        // End View Switcher
    ...
    function initAppPlugins()
    {
        ...
        $(".sc-add-to-cart").on("click", function(e) {
            ...
            var pNode = $(this).find($(".fa"));
            pNode.removeClass('fa-cart-plus').addClass('fa-spinner').addClass('fa-spin');
            $.ajax({
                url: $(this).attr('href'),
                type: 'POST',
                data: {
                ...
                complete: function (xhr, textStatus) {
                    if(200 == xhr.status){
                        @include('ecommerce_frontend.layouts.notification', ['message' => trans('theme.notify.item_added_to_cart'), 'type' => 'success', 'icon' => 'check-circle'])
                        // Increase global cart item count by 1
                        increaseCartItem(1);
                    }
                    else if(444 == xhr.status){
                        @include('ecommerce_frontend.layouts.notification', ['message' => trans('theme.notify.item_added_already_in_cart'), 'type' => 'info', 'icon' => 'info-circle'])
                    }
                    else{
                        @include('ecommerce_frontend.layouts.notification', ['message' => trans('theme.notify.failed'), 'type' => 'warning', 'icon' => 'times-circle'])
                    }
                    pNode.addClass('fa-cart-plus').removeClass('fa-spinner').removeClass('fa-spin');
                },
            });
        });
        ...
        $(".product-info-qty-plus").on('click', function(e) {
            ...
            if(node.data('max') > currentVal){
                ...
                node.val(currentVal).change();
                $.ajax({
				    url: "{{ route('api.cart.update') }}",
				    type: 'POST',
				    dataType: 'JSON',
                    data: {'cart': $(this).closest('form').find('input[name=cart_id]').val(), 
                        'snap_item': $(this).data()['item'],
                        'snap_cart': $(this).data()['cart'],
                        'quantity': currentVal
                    },
				    success: function (data, textStatus, xhr) {
				    	if(200 == xhr.status){
							@include('ecommerce_frontend.layouts.notification', ['message' => trans('theme.notify.coupon_applied'), 'type' => 'success', 'icon' => 'check-circle'])
				    	}
				    }
				})
            }
            else{
                @include('ecommerce_frontend.layouts.notification', ['message' => trans('theme.notify.max_item_stock'), 'type' => 'warning', 'icon' => 'times-circle'])
            }
        });
        ...
        $(".product-info-qty-minus").on('click', function(e) {
            ...
            if (currentVal > node.data('min')){
                ...
                @include('ecommerce_frontend.layouts.notification', ['message' => trans('theme.notify.minimum_order_qtt_reached'), 'type' => 'warning', 'icon' => 'times-circle'])
            }
        });
