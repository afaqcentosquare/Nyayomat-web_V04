	    ...
	    <div class="box-header with-border">
	      ...
	      <div class="box-tools pull-right">
			@can('create', App\Merchant::class)
				<a href="#" data-link="{{ route('admin.vendor.merchant.create') }}" class="ajax-modal-btn btn btn-success btn-flat">{{ trans('app.add_merchant') }}</a>
			@endcan
	      </div>
	    </div> <!-- /.box-header -->
	    <div class="box-body">
			<table class="table table-hover table-2nd-short">
			    ...
			    <!--        		@unless($merchant->active)-->
				   ...
				   <!--       </td>-->
				          <td>
							@can('view', $merchant)
					            <a href="#" data-link="{{ route('admin.vendor.merchant.show', $merchant->id) }}" class="ajax-modal-btn">{{ $merchant->id }}</a>
							@else
					          	{{ $merchant->id }}
							@endcan
				          </td>
				          ...
				          <td>
				              <a href="{{ route('admin.user.secretLogin', $merchant) }}" style="text-decoration:underline;">@php echo $orders = DB::table('orders')->where("shop_id",$merchant->shop_id)->count(); @endphp</a>
				          </td>
				          <td>
				              @php
				                ...
				                $locations = explode(',',$merchant->location);

				                echo "<ul>";

				                foreach($locations as $loc) {

				                    $name = DB::table('in_location')->where('id',$loc)->get();
				          ...
				          <td>
				              <a href="{{ route('admin.user.secretLogin', $merchant) }}" style="text-decoration:underline;">@php echo DB::table('coupons')->where("shop_id",$merchant->shop_id)->count(); @endphp</a>
				          </td>
