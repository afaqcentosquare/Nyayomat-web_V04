<style>
  ...
  @media (max-width: 550px){
    [data-show="show-for-large"], .show-for-large {
        display: none !important;
    }
  }
...
<section>
  <div class="container">
    ...
    $shipping_country_id = $shipping_address ? $shipping_address->country_id :
    config('system_settings.address_default_country');
    $shipping_state_id = $shipping_address ? $shipping_address->state_id :
    config('system_settings.address_default_state');
    }
    else{
    ...
    $country_dropdown .= '<option value="' . $country_id . '" ';
          $country_dropdown .= $country_id == $shipping_country_id ? ' selected' : '' ; $country_dropdown .='>' .
      $country_name . '</option>' ; } @endphp <div class="row">
      <div class="col-sm-12 text-right">
        <a href="{{route('emptycart')}}" class="btn btn-success btn-sm flat pull-right nyayom-btn"><i
            class="fa fa-shopping-cart"></i>Empty Cart</a>
      </div>
  </div>
  ...
  <div class="row shopping-cart-table-wrap space30 {{$expressId == $cart->id ? 'selected' : ''}}"
    ...
    <div class="col-md-9 nopadding">
      <div class="shopping-cart-header-section">
        ...
        <a href="{{ route('show.store', $shop_name->slug) }}"> {{ $shop_name->name }}</a>
      ...
      <table class="table table shopping-cart-item-table" id="table{{$cart->id}}">
        <thead>
          <tr>
            ...
            <th class = "show-for-large">{{ trans('theme.price') }}</th>
        ...
        <tbody>
          ...
          <tr class="cart-item-tr">
            <td>
              ...
              <img src="{{ get_storage_file_url(optional($item->image)->path, 'mini') }}" alt="{{ $item->slug }}" class="cartImage" title="{{ $item->slug }}" />
            </td>
            <td>
              <div class="shopping-cart-item-title">
                <a href="{{ route('show.product', $item->slug) }}"
                  class="product-info-title">{{ $item->pivot->item_description }}</a>
              </div>
            </td>
            <td class="shopping-cart-item-price show-for-large">
              <span>KSh
                <span id="item-price{{$cart->id}}-{{$item->id}}"
                  data-value="{{$unit_price}}">{{ number_format($unit_price, 2, '.', '') }}</span>
              </span>
            </td>
            <td>
              <div class="product-info-qty-item">
                <button class="product-info-qty product-info-qty-minus" data-cart="{{$cart->id}}" data-item="{{$item->id}}">-</button>
                <input name="quantity[{{$item->id}}]" id="itemQtt{{$item->id}}"
                  class="product-info-qty product-info-qty-input" data-cart="{{$cart->id}}" data-item="{{$item->id}}"
                  data-min="{{$item->min_order_quantity}}" data-max="{{$item->stock_quantity}}" type="text"
                  value="{{$item->pivot->quantity}}">
                <button class="product-info-qty product-info-qty-plus" data-cart="{{$cart->id}}" data-item="{{$item->id}}">+</button>
              </div>
            </td>
            <td>
              <span>KSh
            ...
            <td>
              <a class="cart-item-remove" href="javascript:void(0)" data-cart="{{$cart->id}}" data-item="{{$item->id}}"
                data-toggle="tooltip" title="@lang('theme.remove_item')">&times;</a>
            </td>
          </tr> <!-- /.order-body -->
          @endforeach
        </tbody>
        ...
        <!--          <button class="btn btn-success flat apply_seller_coupon" type="button" data-cart="{{$cart->id}}">@lang('theme.button.apply_coupon')</button>-->
        <!--        </span>-->
        <!--      </div><!-- /input-group -->
        <!--    </td>-->
        <!--    <td colspan="3"></td>-->
    ...
    <div class="col-md-3 space20">
      <div class="side-widget" id="cart-summary{{$cart->id}}">
        ...
        <ul class="shopping-cart-summary">
          ...
          <li>
            <span>
              <a class="dynamic-shipping-rates" data-toggle="popover" data-cart="{{$cart->id}}"
                data-options="{{ $shipping_options }}" id="shipping-options{{$cart->id}}"
          ...
          <li>
            <span>
              <a class="packaging-options" data-toggle="popover" data-cart="{{$cart->id}}"
                data-options="{{$packaging_options}}" title="{{ trans('theme.packaging') }}">
      ...
      <!--<button class="btn btn-success btn-sm flat pull-right" id="checkout-btn{{$cart->id}}" type="submit"><i-->
      <!--    class="fa fa-shopping-cart"></i> {{ trans('theme.button.buy_from_this_seller') }}</button>&nbsp;&nbsp;&nbsp;-->
  ...
  <div style="display: flex;">
    ...
    <form method="post" action="{{route('cart.checkout_all')}}" style="margin-left:5px">
      @csrf
      <button type="submit" class="btn btn-success flat nyayom-btn">{{ trans('theme.button.checkout_all') }}</button>
    </form>
  </div>

  @else
  <div class="clearfix space50"></div>
  {{-- <a href="{{ route('') }}" class="btn btn-success btn-sm flat">@lang('theme.button.shop_now')</a> --}}
  <form method="post" action="{{route('countrywise')}}">
    @csrf
    <p class="lead text-center space50">
      ...
      <button type="submit" class="btn btn-success btn-sm flat">@lang('theme.button.shop_now')</button>
    </p>
  </form>
