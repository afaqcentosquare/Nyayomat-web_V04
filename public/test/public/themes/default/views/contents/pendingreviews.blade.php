@if($pendingreviews->count() > 0)
  <table class="table" id="buyer-order-table">
      <thead>
          <tr>
            <th colspan="3">Pending review of orders</th>
          </tr>
      </thead>
      <tbody>
            ...
            @foreach($order->inventories as $item)
              <tr class="order-info-head">
                  ...
                  <td width="20%" class="order-amount">
                    <h5><span>@lang('theme.order_amount'): </span>{{ round($order->grand_total) }} KSh</h5>
                    <div class="btn-group" role="group">
                      <a class="btn btn-xs btn-success flat" href="{{ route('order.detail', $order) }}">@lang('theme.button.order_detail')</a>
                      <a class="btn btn-xs btn-success flat" href="{{ route('order.detail', $order) . '#message-section' }}">@lang('theme.button.contact_seller')</a>
                    </div>
                  </td>
              </tr> <!-- /.order-info-head -->
                ...
                <tr class="order-body">
                  <td colspan="2">
                      ...
                      <div class="product-info">
                          ...
                          <div class="order-info-amount">
                              <span>{{ round($item->pivot->unit_price) }} KSh x {{ $item->pivot->quantity }}</span>
                          </div>
                          {{--
                  ...
                  @if($loop->first)
                    <td rowspan="{{ $loop->count }}" class="order-actions">
                      <a href="{{ route('order.track', $order) }}" class="btn btn-success btn-sm btn-block flat">@lang('theme.button.track_order')</a>
                      @if($order->goods_received)
                        <a href="{{ route('order.feedback', $order) }}" class="btn btn-success btn-sm btn-block flat">@lang('theme.button.give_feedback')</a>
                      @else
                        {!! Form::model($order, ['method' => 'PUT', 'route' => ['goods.received', $order]]) !!}
                          {!! Form::button(trans('theme.button.confirm_goods_received'), ['type' => 'submit', 'class' => 'confirm btn btn-success btn-block flat', 'data-confirm' => trans('theme.confirm_action.goods_received')]) !!}
                        {!! Form::close() !!}
                      @endif
