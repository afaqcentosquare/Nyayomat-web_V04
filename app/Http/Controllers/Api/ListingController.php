...
class ListingController extends Controller
{
    ...
    public function shopListing(Request $request, $merchantId)
    {
        $shop = Shop::where('id', $merchantId)->first();

        if (isset($shop)) {
            $listings = Inventory::where('shop_id', $shop->id)->get();
            return ListingResource::collection($listings);
        } else {
            return response()->json(['status' => trans('app.error'), 'message' => 'Merchant details not found!'], 404);
        }
    }
    ...
    public function updateInventory(Request $request)
    {
        ...
        if (isset($inventory)) {
            ...
            if (isset($request->qty_change)) {
                $inventory->stock_quantity += $request->qty_change;
            }
            
            if (isset($request->price)) {
                $inventory->sale_price = $request->price;
            }

            if (isset($request->purchase_price) && isset($inventory->purchase_price)) {
                $inventory->purchase_price = ($inventory->purchase_price + $request->purchase_price) / 2;
            }else if(isset($request->purchase_price) && !isset($inventory->purchase_price)){
                $inventory->purchase_price =  $request->purchase_price;
            }
            $inventory->save();
            return response()->json(['status' => trans('app.success'), 'message' => 'Inventory updated successfully'], 200);
        } else {
            return response()->json(['status' => trans('app.error'), 'message' => 'Inventory not found!'], 404);
        }
    }
}
