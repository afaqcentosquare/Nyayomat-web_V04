...
class ProductUploadController extends Controller
{
	...
	/**
	 * NewSkin Create Product
	 */
	public function NScreateProduct(Request $request)
	{
        ...
        // if($file = $request->hasFile('image_link'))
        // {
        ...
        $subgroup = CategorySubGroup::Where('name', '=', $data['subgroup'])->first();
        $group = CategoryGroup::Where('name', '=', $data['group'])->first();
        ...
        if ($category !== null) {
            ...
            $category->subGroup()->associate($subgroup->id)->save();

            $subgroup = CategorySubGroup::find($subgroup->id);
    
            $subgroup->group()->associate($group->id)->save();

			$shop = Shop::find($shop->id);

			$product->shop()->associate($shop->id)->save();
        }
...
// Upload featured image
if ($data['image_link'])
$product->saveImage($request->file('image_link'), false);
