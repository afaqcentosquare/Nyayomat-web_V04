...
if (!function_exists('get_address_str_from_request_data')) {
    function get_address_str_from_request_data($request)
    {
        ...
        $str[] =is_numeric($request->city) ? get_value_from($request->city, 'in_cities', 'name') : $request->city;
        $str[] =is_numeric($request->region_id) ? get_value_from($request->region_id, 'in_region', 'name') : $request->region_id;

        if($state){
            $str[] = $state . ' ' . $request->zip_code;
        }       
        $str[] = is_numeric($request->country_id) ? get_value_from($request->country_id, 'in_location', 'name') : $request->country_id;
        if ($request->phone) {
...
if (!function_exists('get_formated_decimal')) {
     ...
     *
     * @param  decimal $value
     * @param  bool $trim  remove un wanted zeros after decimal point
     * @param mixed $decimal
     *
     * @return decimal
     */
...
if (!function_exists('get_formated_price')) {
     ...
     * @param mixed|null $decimal
     *
     * @return string        currency tring
     */
