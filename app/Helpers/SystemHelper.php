...
if (!function_exists('getMysqliConnection')) {
    ...
    {
        return mysqli_connect(env('DB_HOST'), env('DB_USERNAME'), env('DB_PASSWORD'), env('DB_DATABASE'), env('DB_PORT'));
    }
}
...
if (!function_exists('crosscheckAndUpdateOldCartInfo')) {
    ...
    function crosscheckAndUpdateOldCartInfo($request, Cart $cart)
    {
        ...
        foreach ($cart->inventories as $item) {
            ...
            if ($item->id == $request->snap_item) {
                $temp_qtt = 1;
                if ($request->quantity && $request->quantity > 0) {
                    $temp_qtt = $request->quantity;
                } else if (isset($item->pivot) && isset($item->pivot->quantity) && $item->pivot->quantity > 0) {
                    $temp_qtt = $item->pivot->quantity;
                }
                ...
                $temp_qtt = 1;
                if (isset($item->pivot) && isset($item->pivot->quantity) && $item->pivot->quantity > 0) {
                    $temp_qtt = $item->pivot->quantity;
                }
