...
class ListHelper
{
    ...
    {
        return City::where('deleted_at', null)->get()->pluck('name', 'id');
    }
     ...
     * @param mixed $count
     */
    public static function bottom_listing_items($shop = null, $count = 5)
    {
        if (Auth::user()->isFromPlatform() && $shop) {
            $items = Inventory::where('inventories.shop_id', $shop);
        } else {
        ...
        ->join('products', 'inventories.product_id', 'products.id')
        ->join('order_items', 'inventories.id', 'order_items.inventory_id')
        ->groupBy('inventory_id')->orderBy('sold_qtt', 'asc')->limit($count)->get();
    }
     ...
     * @param mixed $count
     */
    public static function all_transactions($shop = null, $count = 5)
    {
        if (Auth::user()->isFromPlatform() && $shop) {
            $transactions = Order::where('orders.shop_id', $shop);
        
        } else {
            $transactions = Order::where('orders.shop_id', Auth::user()->merchantId());
        }
    ...
    {
        $variants = Inventory::select('id', 'slug', 'title', 'stock_quantity', 'condition', 'sale_price', 'offer_price', 'offer_start', 'offer_end', 'min_order_quantity')->where('product_id', $item->product_id)->available();
    ...
    {
        return  Location::where('deleted_at', null)->get()->pluck('name', 'id');
    }
    ...
    /**
     * Get location based category listing for active products.
     *
     * @return array
     */
    public static function location_categories($location_id)
    {
        return \DB::table('location_user')
            ->join('shops', 'location_user.user_id', '=', 'shops.owner_id')->where('shops.active', '=', '1')->where('location_user.location_id', '=', $location_id)
            ->join('inventories', 'shops.id', '=', 'inventories.shop_id')->where('inventories.active', '=', '1')->where('inventories.stock_quantity', '>', 0)
            ->join('category_product', 'inventories.product_id', '=', 'category_product.product_id')
            ->join('categories', 'category_product.category_id', '=', 'categories.id')->where('categories.active', '=', 1)
            ->select('categories.id')
            ->distinct()
            ->get();
    }

    public static function locationCategories($location_id)
    {
        return \DB::table('location_user')
            ->join('shops', 'location_user.user_id', '=', 'shops.owner_id')->where('shops.active', '=', '1')->where('location_user.location_id', '=', $location_id)
            ->join('inventories', 'shops.id', '=', 'inventories.shop_id')->where('inventories.active', '=', '1')->where('inventories.stock_quantity', '>', 0)
    ...
    {
        return  Region::where('deleted_at', null)->get()->pluck('name', 'id');
    }

    public static function region($region_id)
    {
        return Region::where('deleted_at', null)->where('id', $region_id)->first();
    }
}
